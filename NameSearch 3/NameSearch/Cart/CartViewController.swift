...
class CartViewController: UIViewController {
    ...
    static func createVC() -> CartViewController? {
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "\(CartViewController.self)") as? CartViewController
        return vc
    }
    ...
    func updatePayButton() {
        if PaymentsManager.shared.selectedPaymentMethod == nil {
            ...
            ShoppingCart.shared.domains.forEach {
                let priceDouble = Double($0.price.replacingOccurrences(of: "$", with: "").replacingOccurrences(of: ",", with: "")) ?? 0.00
